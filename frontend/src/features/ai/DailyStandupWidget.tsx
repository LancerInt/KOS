...
import {
  ...
  IconButton, Menu, MenuItem, Paper, Stack, Tooltip, Typography,
} from "@mui/material";
...
/**
 * The Daily Stand-Up panel on the Dashboard.
 *
 * Reads from storage on mount — the stand-up is generated by the 9am Celery job,
 * so opening the dashboard costs nothing and makes no provider call. The two
 * buttons are deliberately different: **Refresh** re-reads what the server has,
 * **Regenerate** spends a fresh provider call and is the only control that does.
 *
 * When the server hands generation to a Celery worker it answers `queued`, and
 * this polls until the stand-up lands. With no worker running the server
 * generates inline and returns it immediately, so both deployments behave the
 * same from here.
 */
...
export default function DailyStandupWidget() {
  ...
  // Survives unmount so a poll started before navigating away stops cleanly.
    ...
    <>
      <Paper sx={{ borderRadius: "8px", overflow: "hidden" }}>
        ...
        <Stack
          ...
          sx={{ px: 2, py: 1.35, borderBottom: `1px solid ${tokens.line}`, bgcolor: tokens.paper }}
        >
          <AutoAwesomeRoundedIcon sx={{ fontSize: 17, color: tokens.kriya }} />
          <Typography sx={{ fontFamily: '"Manrope Variable"', fontSize: 13.5, fontWeight: 600, flex: 1 }}>
            Daily Stand-Up
          </Typography>

          {standup && (
            <Typography sx={{ fontSize: 11, fontFamily: monoFont, color: tokens.text3 }}>
          ...
          {loading ? (
            ...
            <Stack spacing={1.5} alignItems="flex-start" sx={{ py: 1 }}>
              ...
              <Typography sx={{ fontSize: 12, color: tokens.text3 }}>
                One is written for you automatically each morning.
              </Typography>
            </Stack>
          )}
        ...
        <Stack
          ...
          sx={{ px: 2, py: 1.25, borderTop: `1px solid ${tokens.line}` }}
        >
